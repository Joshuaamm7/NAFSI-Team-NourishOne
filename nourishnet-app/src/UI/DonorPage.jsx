...
function DonorPage() {
  ...
  return (
    <div className="donor-root">
      ...
      <section className="donor-section anim-fade-up anim-d1">
        <h2 className="donor-section-title">{t('ui.whatDonating')}</h2>
        <div className="donor-upload-row">
          <button className="donor-upload-box"><span className="donor-upload-icon">📷</span><span className="donor-upload-label">{t('ui.takePhoto')}</span></button>
          <button className="donor-upload-box"><span className="donor-upload-icon">📝</span><span className="donor-upload-label">{t('ui.typeItems')}</span></button>
        </div>
      ...
      <section className="donor-section anim-fade-up anim-d2">
        <div className="donor-section-row">
          ...
          <button className="donor-section-arrow"><Arrow /></button>
        </div>
        <div className="donor-hscroll" ref={scrollRef} {...drag(scrollRef)}>
          {highNeed.slice(0, 20).map((loc) => (
            <DonorLocCard key={loc.id} loc={loc} t={t} />
          ))}
        </div>
      </section>
      ...
      <section className="donor-section anim-fade-up anim-d3">
        ...
        <div className="donor-hscroll" ref={foodRef} {...drag(foodRef)}>
          {FOOD_TYPES.slice(0, 12).map((ft) => (
            <button key={ft} className="donor-green-card"><span className="donor-green-label">{ft}</span></button>
          ))}
        </div>
      </section>
      ...
      <section className="donor-section anim-fade-up anim-d4">
        ...
        <div className="donor-impact-grid">
          <div className="donor-impact-card"><span className="donor-impact-value">{meals}</span><span className="donor-impact-label">{t('impact.meals')}</span></div>
          <div className="donor-impact-card"><span className="donor-impact-value">{co2} lbs</span><span className="donor-impact-label">{t('impact.co2')}</span></div>
          <div className="donor-impact-card"><span className="donor-impact-value">{water} gal</span><span className="donor-impact-label">{t('impact.water')}</span></div>
        </div>
      </section>
    </div>
  );
...
function DonorLocCard({ loc, t }) {
  return (
    <div className="donor-loc-card">
      ...
      <button className="donor-card-donate-btn">{t('ui.donateBtn')}</button>
    </div>
  );
}
