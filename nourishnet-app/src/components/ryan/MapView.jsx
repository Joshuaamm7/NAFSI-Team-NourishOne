...
function AutoFitBounds({ locations }) {
  const map = useMap();
  useEffect(() => {
    // Fix for blank tiles — tell Leaflet to recalculate container size
...
>>>>>>> Stashed changes
        ...
        {validLocations.map((loc, index) => {
          ...
          const badges = loc.healthAttributes
            ? Object.entries(loc.healthAttributes)
                .filter(([, v]) => v)
                .map(([k]) => HEALTH_LABELS[k])
                .filter(Boolean)
            : [];
          ...
          return (
            <Marker
              ...
              icon={createColoredIcon(loc.type)}
            >
              <Popup>
                <strong>{loc.name}</strong>
                ...
                {badges.length > 0 && (
                  <p style={{ margin: '4px 0 0', fontSize: '0.8em' }}>{badges.join(' · ')}</p>
                )}
                <a
